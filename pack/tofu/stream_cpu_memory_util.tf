...
data "apstra_blueprints" "all_bps" {}

resource "apstra_raw_json" "stream_cpu_memory_util" {
    for_each = data.apstra_blueprints.all_bps.ids
    url = format("/api/blueprints/%s/probes", each.key)
    payload = <<-EOF
    {
